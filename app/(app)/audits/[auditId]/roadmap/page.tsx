...
function formatMonth(d: Date): string { return d.toLocaleDateString('en-GB', { month: 'short', year: '2-digit' }); }
...
export default function AuditRoadmapPage() {
  ...
  const visiblePocs = showPocs ? datedPocs.filter(p => pocPhaseFilter === 'all' || p.phase === pocPhaseFilter) : [];
  const visibleInds = showInds ? datedInds.filter(i => indStatusFilter === 'all' || i.status === indStatusFilter) : [];
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-display font-bold text-text">Audit Roadmap</h1>
          <span className="text-muted text-sm">— {pocs.length} POCs · {inds.length} Industrializations</span>
        </div>
        <div className="flex gap-2">
          <button onClick={() => router.push(`/audits/${auditId}/pocs`)} className="btn-secondary flex items-center gap-1 text-xs">
            <ExternalLink size={13} /> POCs
          </button>
          <button onClick={() => router.push(`/audits/${auditId}/industrializations`)} className="btn-secondary flex items-center gap-1 text-xs">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <label className="flex items-center gap-1.5 bg-white border border-border rounded-sm px-3 py-1.5 text-xs cursor-pointer">
          <input type="checkbox" checked={showPocs} onChange={e => setShowPocs(e.target.checked)} className="accent-teal-poc" /> POCs
        </label>
        <label className="flex items-center gap-1.5 bg-white border border-border rounded-sm px-3 py-1.5 text-xs cursor-pointer">
          <input type="checkbox" checked={showInds} onChange={e => setShowInds(e.target.checked)} className="accent-indu" /> Industrializations
        </label>
        {showPocs && (
      ...
      {empty ? (
        ...
        <div ref={containerRef} className="bg-white border border-border rounded-sm overflow-x-auto">
          ...
          <div style={{ width: LABEL_W + ganttW }}>
            ...
            {groupBy === 'list' ? (
              <>
                {visiblePocs.map(poc => {
                  ...
                  const ms = (poc.execution?.milestones ?? []).filter(m => m.dueDate);
                  return (
                    <div key={poc._id}>
                      <RowPOC
                        poc={poc}
                      ...
                      {ind && (
                        <RowInd
                          ...
                          todayPct={todayPct}
                          indented
                          onClick={() => router.push(`/audits/${auditId}/industrializations/${ind._id}`)}
                        />
                      )}
                    </div>
                  );
              ...
              renderByUseCase({ visiblePocs, visibleInds, ganttStart, totalMonths, ganttW, monthWidth, months, showToday, todayPct, auditId, router })
            )}
            ...
            {(() => {
              const hidden = pocs.filter(p => !p.design?.startDate || !p.design?.deadlineDate).length
...
interface RowProps {
  ganttStart: Date; totalMonths: number; ganttW: number; monthWidth: number; months: Date[];
  showToday: boolean; todayPct: number; onClick: () => void;
}

function RowPOC({ poc, milestones, indented, ...r }: RowProps & { poc: POCRow; milestones: Milestone[]; indented?: boolean }) {
  ...
  const procLabel = proc?.procId ? `${proc.procId}${proc.name ? ' · ' + proc.name : ''}` : '';
  const objective = poc.design?.measurableObjective ?? '';
  return (
    <div className="flex border-b border-border/50 hover:bg-slate-50/50 cursor-pointer group transition-colors" style={{ minHeight: milestones.length > 0 ? 72 : 52 }} onClick={r.onClick}>
      <div className={`flex-shrink-0 ${indented ? 'pl-8 pr-4' : 'px-4'} py-3 border-r border-border flex flex-col justify-center gap-0.5`} style={{ width: LABEL_W }}>
        <div className="flex items-center gap-1.5 flex-wrap">
          <span className="font-mono text-xs font-bold text-teal-poc">{poc.pocId}</span>
      ...
      <div className="relative py-3" style={{ width: r.ganttW }}>
        ...
        {r.showToday && <div className="absolute top-0 bottom-0 pointer-events-none z-10" style={{ left: `${r.todayPct}%`, width: 1, backgroundColor: '#ef4444', opacity: 0.5 }} />}
        <div
          className="absolute rounded-sm flex items-center px-2 overflow-hidden group-hover:opacity-90"
          style={{ left: `${leftPct}%`, width: `${widthPct}%`, minWidth: 6, height: 20, top: milestones.length > 0 ? 'calc(50% - 22px)' : 'calc(50% - 10px)', backgroundColor: POC_PHASE_COLORS[poc.phase] }}
        ...
        <Markers milestones={milestones} ganttStart={r.ganttStart} totalMonths={r.totalMonths} />
      </div>
    </div>
  );
}

function RowInd({ ind, indented, ...r }: RowProps & { ind: IndRow; indented?: boolean }) {
  ...
  return (
    <div
      ...
      style={{ minHeight: ms.length > 0 ? 60 : 44, backgroundColor: indented ? 'rgba(255,237,213,0.25)' : undefined }}
      onClick={r.onClick}
    >
      <div className={`flex-shrink-0 ${indented ? 'pl-8 pr-4' : 'px-4'} py-2 border-r border-border flex flex-col justify-center gap-0.5`} style={{ width: LABEL_W }}>
        <div className="flex items-center gap-1.5 flex-wrap">
        ...
        {!indented && poc?.pocId && <p className="text-[10px] text-muted font-mono">from {poc.pocId}</p>}
      </div>
      <div className="relative py-3" style={{ width: r.ganttW }}>
        <Grid months={r.months} monthWidth={r.monthWidth} />
        {r.showToday && <div className="absolute top-0 bottom-0 pointer-events-none z-10" style={{ left: `${r.todayPct}%`, width: 1, backgroundColor: '#ef4444', opacity: 0.5 }} />}
        <div
          className="absolute rounded-sm flex items-center px-2 overflow-hidden group-hover:opacity-90"
          style={{ left: `${leftPct}%`, width: `${widthPct}%`, minWidth: 6, height: 20, top: ms.length > 0 ? 'calc(50% - 22px)' : 'calc(50% - 10px)', backgroundColor: IND_STATUS_COLORS[ind.status] }}
        ...
        <Markers milestones={ms} ganttStart={r.ganttStart} totalMonths={r.totalMonths} />
      </div>
    </div>
  );
}
...
function Grid({ months, monthWidth }: { months: Date[]; monthWidth: number }) {
  return (
    <div className="absolute inset-0 flex pointer-events-none">
      {months.map((_, i) => <div key={i} style={{ width: monthWidth, flexShrink: 0 }} className="border-r border-border/20 h-full" />)}
    </div>
  );
}

function Markers({ milestones, ganttStart, totalMonths }: { milestones: Milestone[]; ganttStart: Date; totalMonths: number }) {
  return (
    <>
      {milestones.filter(ms => ms.dueDate).map(ms => {
        ...
        return (
          <div key={ms.id} className="absolute flex flex-col items-center" style={{ left: `calc(${pct}% - 5px)`, top: 'calc(50%)' }} title={`${ms.name}\n${date} · ${ms.status}`}>
...
interface UCArgs {
  visiblePocs: POCRow[]; visibleInds: IndRow[]; ganttStart: Date; totalMonths: number;
  ganttW: number; monthWidth: number; months: Date[]; showToday: boolean; todayPct: number;
  auditId: string; router: ReturnType<typeof useRouter>;
}

function renderByUseCase({ visiblePocs, visibleInds, ganttStart, totalMonths, ganttW, monthWidth, months, showToday, todayPct, auditId, router }: UCArgs) {
  const groups = new Map<string, { cuId: string; description?: string; pocs: POCRow[]; inds: IndRow[] }>();

  for (const p of visiblePocs) {
    const ucKey = (typeof p.useCaseId === 'object' ? p.useCaseId._id : String(p.useCaseId)) ?? p._id;
    const uc = typeof p.useCaseId === 'object' ? p.useCaseId : null;
    if (!groups.has(ucKey)) groups.set(ucKey, { cuId: uc?.cuId ?? '—', description: uc?.description, pocs: [], inds: [] });
    groups.get(ucKey)!.pocs.push(p);
  }
  for (const i of visibleInds) {
    const ucKey = typeof i.useCaseId === 'object' ? i.useCaseId._id : String(i.useCaseId);
  ...
  return [...groups.entries()].map(([ucId, g]) => {
    ...
    for (const p of g.pocs) for (const m of (p.execution?.milestones ?? [])) if (m.dueDate) allMs.push(m as Milestone);
    for (const i of g.inds) for (const m of (i.milestones ?? [])) if (m.dueDate) allMs.push(m as Milestone);
    ...
    const target = ind ? `/audits/${auditId}/industrializations/${ind._id}`
      : poc ? `/audits/${auditId}/pocs/${poc._id}` : null;
    ...
    return (
      ...
      >
        <div className="flex-shrink-0 px-4 py-2 border-r border-border flex flex-col justify-center gap-0.5" style={{ width: LABEL_W }}>
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="font-mono text-xs font-bold text-purple-aria">{g.cuId}</span>
            {g.pocs.map(p => <Badge key={p._id} variant={POC_PHASE_VARIANTS[p.phase]}>POC</Badge>)}
            {g.inds.map(i => <Badge key={i._id} variant={IND_STATUS_VARIANTS[i.status]}>IND</Badge>)}
          </div>
          {g.description && <p className="text-xs text-text truncate" title={g.description}>{g.description}</p>}
        </div>
        <div className="relative py-3" style={{ width: ganttW }}>
          <Grid months={months} monthWidth={monthWidth} />
          {showToday && <div className="absolute top-0 bottom-0 pointer-events-none z-10" style={{ left: `${todayPct}%`, width: 1, backgroundColor: '#ef4444', opacity: 0.5 }} />}
          {connector && (
            <div
              className="absolute"
              style={{ left: `${connector.fromPct}%`, width: `${connector.toPct - connector.fromPct}%`, top: 'calc(50% - 1px)', height: 2, background: 'repeating-linear-gradient(90deg, #94a3b8 0 4px, transparent 4px 8px)' }}
          ...
          {g.pocs.map(p => {
            ...
            return (
              <div key={p._id}
                ...
                style={{ left: `${lp}%`, width: `${Math.max(rp - lp, 0.5)}%`, minWidth: 6, height: 20, top: hasMs ? 'calc(50% - 22px)' : 'calc(50% - 10px)', backgroundColor: POC_PHASE_COLORS[p.phase] }}
                title={`${p.pocId} · ${s.toLocaleDateString('en-GB')} → ${e.toLocaleDateString('en-GB')}`}
              >
                <span className="text-white text-[10px] font-medium truncate">{p.pocId}</span>
              </div>
            );
          })}
          {g.inds.map(i => {
            ...
            return (
              <div key={i._id}
                className="absolute rounded-sm flex items-center px-2 overflow-hidden"
                style={{ left: `${lp}%`, width: `${Math.max(rp - lp, 0.5)}%`, minWidth: 6, height: 20, top: hasMs ? 'calc(50% - 22px)' : 'calc(50% - 10px)', backgroundColor: IND_STATUS_COLORS[i.status] }}
              ...
              >
                <span className="text-white text-[10px] font-medium truncate">{i.industrializationId}</span>
              </div>
            );
          })}
          <Markers milestones={allMs} ganttStart={ganttStart} totalMonths={totalMonths} />
        </div>
      </div>
    );
  });
