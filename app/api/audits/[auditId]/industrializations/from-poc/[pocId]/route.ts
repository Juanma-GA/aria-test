...
export async function POST(
  ...
  try {
    ...
    if (existing) {
      return NextResponse.json(
        { error: 'An industrialization already exists for this POC', industrializationId: (existing as any)._id },
        { status: 409 }
      );
    }
    ...
    } else if (sourceDesign.responsibleUserId && mongoose.isValidObjectId(sourceDesign.responsibleUserId)) {
      const user = await User.findById(sourceDesign.responsibleUserId).select('name email').lean() as any;
      resolvedTechnicalLead = user?.name || user?.email || '';
    }

    const aiGenerated: string[] = ['plan.scope', 'plan.sovereigntyConstraints', 'cost.oneTime.developmentEur'];
    if (!body.plan?.ownerTechnical && resolvedTechnicalLead) aiGenerated.push('plan.ownerTechnical');
    ...
    const carriedBreakdown = sourceBreakdown && (sourceBreakdown as any).mode
      ? {
      ...
      : undefined;
    if (carriedBreakdown) aiGenerated.push('cost.recurringAnnual.computeBreakdown');
    ...
    const created = await Industrialization.create({
      ...
      plan: {
        ...
        sovereigntyConstraints: body.plan?.sovereigntyConstraints ?? sourceDesign.activeB2Restrictions ?? '',
        dependencies: body.plan?.dependencies ?? '',
      },
      cost: {
        oneTime: {
          developmentEur: sourceEval.actualCostEur ?? sourceDesign.estimatedDevCostEur ?? 0,
        },
