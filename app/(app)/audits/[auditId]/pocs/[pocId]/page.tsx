...
export default function POCDetailPage() {
  ...
  const [confirmDelete, setConfirmDelete] = useState<{
    ...
    indCount: number;
    error?: string;
  }>({ open: false, cascade: false, indCount: 0 });
  ...
  const updateCriterion = (id: string, field: string, value: unknown) => {
    if (!poc) return;
    const criteria = (poc.design?.successCriteria || []).map(
      (c: POCCriterion) => (c.id === id ? { ...c, [field]: value } : c),
    );
    const next = {
      ...poc,
      design: { ...poc.design, successCriteria: criteria },
    };
    setPoc(next as POC);
    trigger({ design: next.design });
  };
  ...
  const addCriterion = () => {
    ...
    const next = {
      ...
      design: {
        ...poc.design,
        successCriteria: [...(poc.design?.successCriteria || []), criterion],
      },
    };
    setPoc(next as POC);
    trigger({ design: next.design });
  ...
  const removeCriterion = (id: string) => {
    if (!poc) return;
    const criteria = (poc.design?.successCriteria || []).filter(
      (c: POCCriterion) => c.id !== id,
    );
    const next = {
      ...poc,
      design: { ...poc.design, successCriteria: criteria },
  ...
  const addMilestone = () => {
    ...
    const next = {
      ...poc,
      execution: { ...poc.execution, milestones: [...milestones, m] },
    };
    setPoc(next as POC);
    trigger({ execution: next.execution });
  };
  ...
  const updateMilestone = (id: string, field: string, value: unknown) => {
    if (!poc) return;
    const milestones = (poc.execution?.milestones ?? []).map(
      (m: POCMilestone) => (m.id === id ? { ...m, [field]: value } : m),
    );
  ...
  const updateCriterionResult = (
    ...
    if (!poc) return;
    const criteria = (poc.design?.successCriteria || []).map(
      (c: POCCriterion) => (c.id === id ? { ...c, actualResult, passed } : c),
    );
    const next = {
      ...poc,
      design: { ...poc.design, successCriteria: criteria },
  ...
  const requestDelete = () =>
    setConfirmDelete({ open: true, cascade: false, indCount: 0 });
  ...
  const handleDelete = async () => {
    ...
    if (res.status === 409 && data?.dependents) {
      setConfirmDelete({
        open: true,
        cascade: true,
        indCount: data.dependents.industrializations ?? 0,
        error: data.error,
      });
    } else {
      setConfirmDelete((s) => ({
  ...
  const aggregateProgress =
    milestones.length === 0
      ? 0
      : Math.round(
          milestones.reduce((s, m) => s + milestonePct(m as any), 0) /
            milestones.length,
        );
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <span className="font-mono text-sm text-teal-poc font-semibold">
            {poc.pocId}
          </span>
          {poc.name && (
        ...
        <div className="flex items-center gap-3">
          ...
          {canEdit && (
            <button
              ...
              title={(poc as any).isArchived ? 'Unarchive' : 'Archive'}
            >
              {(poc as any).isArchived ? (
      ...
      {(() => {
        ...
        if (proc && typeof proc === 'object' && proc.procId)
          nodes.push({
            ...
            code: proc.procId,
            label: proc.name,
            href: `/audits/${auditId}/processes/${typeof proc === 'object' ? ((proc as any)._id ?? '') : proc}`,
          });
      ...
      <fieldset disabled={!canEdit} className="contents">
        ...
        <div className="flex gap-1 border-b border-border mb-5">
          {PHASES.map((p) => {
            ...
            const isDone =
              currentPhaseIdx > PHASES.findIndex((x) => x.key === p.key);
            return (
              <button
                key={p.key}
              ...
              >
                {isDone ? (
                  ...
                  <span
                    className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold border ${isActive ? 'border-teal-poc text-teal-poc' : isCurrent ? 'border-blue-aria text-blue-aria' : 'border-muted text-muted'}`}
                  >
                    {p.num}
                  </span>
                )}
                {p.label}
                {isCurrent && !isActive && (
                  <span className="text-[10px] text-blue-aria ml-1">
                    · current
                  </span>
                )}
              </button>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateDesign('deadlineDate', e.target.value)
                    }
                  />
                </div>
                <div className="col-span-2">
              ...
              <div>
                ...
                {(poc.design?.successCriteria || []).length === 0 ? (
                  <p className="text-sm text-muted">
                    Add at least 2 success criteria.
                  </p>
                ) : (
                  <div className="space-y-3">
                    {(poc.design?.successCriteria as POCCriterion[]).map(
                      (c) => (
                        <div
                          key={c.id}
                          className="border border-border rounded p-3 grid grid-cols-3 gap-3"
                        >
                          <div>
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              ...
                              className="text-muted hover:text-red-sov mt-4"
                            >
                              <Trash2 size={13} />
                            </button>
                          </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-sm font-semibold flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-medium uppercase tracking-wide text-blue-aria bg-blue-pale rounded px-1.5 py-0.5">
                        carries to industrialization
                      </span>
                    </h3>
                    <p className="text-[11px] text-muted leading-snug">
                      Size the POC for its <strong>limited pilot scope</strong>{' '}
                      — typical POC volumes (a handful of users, hundreds–low
                      thousands of executions/year) — using the central model +
                      GPU catalog. The model / GPU / mode choice is the part
                      that usually carries unchanged to production;{' '}
                      <em>annual executions</em> and <em>GPU count</em> are the
                      ones that get <strong>scaled up</strong> on promotion to
                      industrialization. If POC results invalidate the
                      assumptions (e.g. a different model wins, on-prem turns
                      out infeasible), you can change anything in
                      industrialization without losing the inherited starting
                      point.
                    </p>
                  </div>
          ...
          {activeTab === 'execution' && (
            <div className="space-y-4">
              ...
              <div>
                ...
                {milestones.length === 0 ? (
                  <p className="text-sm text-muted">
                    No milestones yet. Add at least one to track progress.
                  </p>
                ) : (
                  <div className="space-y-3">
                    {milestones.map((m) => {
                ...
                {milestones.length > 0 && (
                  <div className="mt-3">
                    ...
                    <div className="w-full h-2 bg-slate-200 rounded-full">
                      <div
                        className="h-2 bg-teal-poc rounded-full transition-all"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      updateExecution('incidents', e.target.value)
                    }
                  />
                </div>
                <div>
          ...
          {activeTab === 'evaluation' && (
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                {(poc.design?.successCriteria || []).length === 0 && (
                  <p className="text-sm text-muted">
                    No success criteria defined in Design phase.
                  </p>
                )}
              </div>
                ...
                {[
                  ...
                  <div key={field}>
                    ...
                    <textarea
                      ...
                      value={
                        (
                          (poc.evaluation || {}) as unknown as Record<
                            string,
                            string
                          >
                        )[field] || ''
            ...
            <div className="space-y-4">
                    ...
                    <textarea
                      ...
                      onChange={(e) => {
                        const n = {
                          ...
                          decision: {
                            ...poc.decision,
                            conditionalRequirement: e.target.value,
                          },
                        };
                        setPoc(n as POC);
                        trigger({ decision: n.decision });
              ...
              {(poc.decision?.decision === 'go' ||
                ...
                <div className="p-3 bg-green-sov-light rounded text-sm text-green-sov flex items-center gap-2">
                  ...
                  <span>
                    GO decision recorded. Update the B7 roadmap initiative with
                    actual POC data.
                  </span>
                  <button
                    onClick={() => router.push(`/audits/${auditId}/roadmap`)}
      ...
      <ConfirmModal
        ...
        title={
          confirmDelete.cascade
            ? 'Delete POC and its industrialization?'
            : 'Delete POC?'
        }
        message={
          confirmDelete.cascade
        ...
        onClose={() =>
          setConfirmDelete({ open: false, cascade: false, indCount: 0 })
        }
      />
    </div>
  );
