...
export default function AuditReportDataPage() {
  ...
  useEffect(() => {
    fetch(apiUrl(`/api/audits/${auditId}/report-data`), { credentials: 'include' })
      .then((r) => r.json())
      .then((data) => {
        if (data.markdown) setMarkdown(data.markdown);
      })
      .catch(() => setError('Error cargando el informe'))
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="font-display text-2xl font-bold text-text flex items-center gap-2">
              <FileText size={22} className="text-blue-aria" />
              Audit Report
            </h1>
            <p className="text-sm text-muted mt-0.5">
              Informe determinista generado a partir de los datos de la auditoría
            </p>
          </div>
        </div>
