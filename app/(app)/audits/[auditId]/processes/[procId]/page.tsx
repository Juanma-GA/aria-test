...
import { Check, Minus, Plus, Pencil, X, FlaskConical, Lightbulb, Factory, ExternalLink } from 'lucide-react';
import { Badge } from '@/components/ui/Badge';
import { Spinner } from '@/components/ui/Spinner';
import { TagInput } from '@/components/ui/TagInput';
import { Modal } from '@/components/ui/Modal';
import type {
  Priority, ProcessStatus, SectorType, BlockCompletion,
  AIType, UseCaseStatus, POCPhase, POCDecisionType,
...
interface POCRow {
  ...
  design?: { measurableObjective?: string; deadlineDate?: string; scopeDescription?: string };
...
const POC_DECISIONS: POCDecisionType[] = ['pending', 'go', 'go_conditional', 'no_go_redesign', 'no_go_discard', 'paused'];
const IND_STATUSES: IndustrializationStatus[] = [
  'pending_customer_validation', 'planned', 'work_in_progress', 'go_for_run', 'stand_by', 'cancelled',
];
    ...
    const load = async () => {
      try {
        const [procRes, ucRes, pocRes, indRes] = await Promise.all([
          ...
          fetch(`/api/audits/${auditId}/industrializations?processId=${procId}`, { credentials: 'include' }),
        ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-display font-semibold text-base text-text flex items-center gap-2">
            ...
            <span className="text-muted font-normal text-sm">({industrializations.length})</span>
          </h2>
          <button
            onClick={() => router.push(`/audits/${auditId}/industrializations/new`)}
            className="flex items-center gap-1 text-xs text-blue-aria border border-blue-aria/30 rounded px-2 py-1 hover:bg-blue-pale transition-colors"
          >
        ...
        {industrializations.length === 0 ? (
          <div className="bg-white border border-border rounded-sm p-8 text-center">
            <p className="text-sm text-muted">
              No industrializations for this process yet. Create one from a validated POC (decision: GO or GO Conditional).
            </p>
          </div>
        ) : (
          <div className="bg-white border border-border rounded-sm overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-slate-50">
                  {['IND ID', 'Name', 'POC', 'Owner', 'Target Go-Live', 'Progress', 'Status'].map((h) => (
                    <th key={h} className="text-left py-2.5 px-4 text-xs font-semibold text-muted uppercase tracking-wide whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {industrializations.map((ind) => {
                  ...
                  const done = (ind.milestones ?? []).filter(m => m.status === 'done').length;
                  ...
                  return (
                    <tr key={ind._id} className="border-b border-border/50 hover:bg-slate-50 cursor-pointer"
                      onClick={() => setEditingInd(ind)}>
                      <td className="py-2.5 px-4">
                        <span className="font-mono text-xs text-indu font-medium">{ind.industrializationId}</span>
                      </td>
                      <td className="py-2.5 px-4 max-w-[200px]">
                        <p className="text-xs text-text truncate">{ind.name || <span className="text-muted">—</span>}</p>
                      </td>
                      <td className="py-2.5 px-4 font-mono text-xs text-teal-poc">{pocRef ?? '—'}</td>
                      <td className="py-2.5 px-4 text-xs">{owner}</td>
                      <td className="py-2.5 px-4 text-xs text-muted">
                        {ind.plan?.targetGoLiveDate ? new Date(ind.plan.targetGoLiveDate).toLocaleDateString('en-GB') : '—'}
                      </td>
                      <td className="py-2.5 px-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                      ...
                      <td className="py-2.5 px-4">
                        <Badge variant={IND_STATUS_VARIANTS[ind.status] ?? 'slate'}>
      ...
      <UseCaseQuickModal
        ...
        onSaved={(updated) => {
          setUseCases((rows) => rows.map(r => r._id === updated._id ? { ...r, ...updated } : r));
          setEditingUC(null);
        }}
      />
      ...
      <POCQuickModal
        ...
        onSaved={(updated) => {
          setPocs((rows) => rows.map(r => r._id === updated._id ? { ...r, ...updated, useCaseId: r.useCaseId } : r));
          setEditingPOC(null);
        }}
      />
      ...
      <IndustrializationQuickModal
        ...
        onSaved={(updated) => {
          setIndustrializations((rows) => rows.map(r => r._id === updated._id ? { ...r, ...updated, pocId: r.pocId, useCaseId: r.useCaseId } : r));
          setEditingInd(null);
        }}
      />
    </div>
...
function UseCaseQuickModal({
  uc, auditId, procId, onClose, onSaved,
}: {
  uc: UCRow | null;
  auditId: string;
  procId: string;
  ...
  const toggleAi = (t: AIType) => {
    setForm(f => f ? {
      ...f,
      aiTypes: f.aiTypes.includes(t) ? f.aiTypes.filter(x => x !== t) : [...f.aiTypes, t],
    } : f);
  };
  ...
  const handleSave = async () => {
    ...
    setError('');
    try {
      const res = await fetch(`/api/audits/${auditId}/usecases/${form._id}`, {
  ...
  const total = uc.score?.dimensions
    ? calculateScore(uc.score.dimensions as Parameters<typeof calculateScore>[0]).total
    : 0;

  return (
    <Modal isOpen={true} onClose={onClose} title={`Use case · ${uc.cuId}`} size="xl">
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <span className="text-xs text-muted">Score: <span className="font-bold text-text">{total}/30</span></span>
          <Link
            href={`/audits/${auditId}/processes/${procId}/b5`}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <select
              ...
              value={form.status}
              onChange={(e) => setForm(f => f ? { ...f, status: e.target.value as UseCaseStatus } : f)}
            >
              {UC_STATUSES.map(s => (
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setForm(f => f ? { ...f, estimatedDevCostEur: Number(e.target.value) } : f)}
            />
          </div>
        </div>
              ...
              return (
                <button
                  ...
                  className={`px-2 py-1 text-[11px] rounded-sm border transition-colors ${
                    checked ? 'bg-blue-aria text-white border-blue-aria' : 'bg-white text-muted border-border hover:border-blue-aria'
                  }`}
                >
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            onChange={(e) => setForm(f => f ? { ...f, requiresClientIT: e.target.checked } : f)}
          />
          <label htmlFor="uc-clientit" className="text-xs text-text">Requires client IT approval</label>
        </div>
...
function POCQuickModal({
  ...
  useEffect(() => {
    setForm(poc ? { ...poc, design: { ...(poc.design ?? {}) }, decision: { ...(poc.decision ?? {}) } } : null);
    setError('');
  }, [poc]);
  ...
  const handleSave = async () => {
    ...
    setError('');
    try {
      const res = await fetch(`/api/audits/${auditId}/pocs/${form._id}`, {
        method: 'PATCH', credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: form.name ?? '',
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title={`POC · ${poc.pocId}`} size="xl">
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <span className="text-xs text-muted">
            Linked UC: <span className="font-mono text-blue-aria">{(poc as any).useCaseId?.cuId ?? '—'}</span>
          </span>
          <Link
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <select
              className="form-input"
              value={form.phase}
              onChange={(e) => setForm(f => f ? { ...f, phase: e.target.value as POCPhase } : f)}
            >
              {POC_PHASES.map(p => (
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setForm(f => f ? { ...f, decision: { ...(f.decision ?? {}), decision: e.target.value as POCDecisionType } } : f)}
            >
              {POC_DECISIONS.map(d => (
        ...
        <div>
          ...
          <textarea
            ...
            onChange={(e) => setForm(f => f ? { ...f, design: { ...(f.design ?? {}), measurableObjective: e.target.value } } : f)}
          />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            onChange={(e) => setForm(f => f ? { ...f, design: { ...(f.design ?? {}), scopeDescription: e.target.value } } : f)}
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input
              ...
              onChange={(e) => setForm(f => f ? { ...f, design: { ...(f.design ?? {}), deadlineDate: e.target.value || undefined } } : f)}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => setForm(f => f ? { ...f, decision: { ...(f.decision ?? {}), justification: e.target.value } } : f)}
            />
          </div>
        </div>
...
function IndustrializationQuickModal({
  ...
  const handleSave = async () => {
    ...
    setError('');
    try {
      const res = await fetch(`/api/audits/${auditId}/industrializations/${form._id}`, {
        method: 'PATCH', credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: form.name ?? '',
  ...
  const pocRef = typeof ind.pocId === 'object' ? ind.pocId?.pocId : undefined;
  const ucRef = typeof ind.useCaseId === 'object' ? ind.useCaseId?.cuId : undefined;

  return (
    <Modal isOpen={true} onClose={onClose} title={`Industrialization · ${ind.industrializationId}`} size="xl">
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <span className="text-xs text-muted">
            POC <span className="font-mono text-teal-poc">{pocRef ?? '—'}</span>
            {' · '}UC <span className="font-mono text-blue-aria">{ucRef ?? '—'}</span>
          </span>
          <Link
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <select
              className="form-input"
              value={form.status}
              onChange={(e) => setForm(f => f ? { ...f, status: e.target.value as IndustrializationStatus } : f)}
            >
              {IND_STATUSES.map(s => (
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setForm(f => f ? { ...f, plan: { ...(f.plan ?? {}), targetGoLiveDate: e.target.value || undefined } } : f)}
            />
          </div>
        </div>
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => setForm(f => f ? { ...f, statusReason: e.target.value } : f)}
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input
              ...
              onChange={(e) => setForm(f => f ? { ...f, plan: { ...(f.plan ?? {}), ownerBusiness: e.target.value } } : f)}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => setForm(f => f ? { ...f, plan: { ...(f.plan ?? {}), ownerTechnical: e.target.value } } : f)}
            />
          </div>
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            onChange={(e) => setForm(f => f ? { ...f, plan: { ...(f.plan ?? {}), scope: e.target.value } } : f)}
          />
        </div>
