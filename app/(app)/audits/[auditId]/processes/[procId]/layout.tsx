...
export default function ProcessLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    Promise.all([
      fetch(`/api/audits/${auditId}`, { credentials: 'include' }).then(r => r.ok ? r.json() : null).catch(() => null),
      fetch(`/api/audits/${auditId}/processes/${procId}`, { credentials: 'include' }).then(r => r.ok ? r.json() : null).catch(() => null),
    ]).then(([audit, proc]) => {
      if (!active) return;
