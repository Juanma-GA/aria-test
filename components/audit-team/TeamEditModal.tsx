...
export function TeamEditModal({ isOpen, onClose, auditId, onChanged }: Props) {
  ...
  const refresh = async () => {
    const [teamRes, usersRes, accessRes] = await Promise.all([
      fetch(apiUrl(`/api/audits/${auditId}/team`), {
        credentials: 'include',
      }).then((r) => r.json()),
      fetch(apiUrl('/api/users'), { credentials: 'include' }).then((r) =>
        r.json(),
      ),
      fetch(apiUrl(`/api/audits/${auditId}/access`), {
        credentials: 'include',
      }).then((r) => r.json()),
    ]);
    const team: TeamPayload[] = Array.isArray(teamRes?.team)
      ? teamRes.team
      : [];
    setMembers(
      team.map((m) => ({ userId: m.userId, role: m.role, user: m.user })),
    );
  ...
  const handleAdd = async (userId: string, role: AuditTeamRole) => {
    setError('');
    try {
      const res = await fetch(apiUrl(`/api/audits/${auditId}/team`), {
        method: 'POST',
        credentials: 'include',
  ...
  const handleUpdateRole = async (userId: string, role: AuditTeamRole) => {
    setError('');
    try {
      const res = await fetch(apiUrl(`/api/audits/${auditId}/team/${userId}`), {
        method: 'PATCH',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        ...
        <div className="space-y-3">
          {!canManage && (
            <div className="text-xs text-muted bg-smoke border border-border rounded-sm px-3 py-2">
              You can see the team but only audit owners can add or change
              members.
            </div>
          )}
          {error && (
