...
export default function IndustrializationDetailPage() {
  ...
  const [aiBusy, setAiBusy] = useState<
    null | 'milestones' | 'cost' | 'maintenance'
  >(null);
  ...
  useEffect(() => {
    Promise.all([
      fetch(apiUrl(`/api/audits/${auditId}/industrializations/${indId}`), {
        credentials: 'include',
      }).then((r) => r.json()),
  ...
  const save = useCallback(
    async (updated: Partial<Industrialization>) => {
      setSaveStatus('saving');
      try {
        const res = await fetch(
          apiUrl(`/api/audits/${auditId}/industrializations/${indId}`),
          {
            method: 'PATCH',
            credentials: 'include',
  ...
  const updateMilestone = (id: string, field: string, value: unknown) => {
    if (!ind) return;
    const milestones = (ind.milestones ?? []).map((m) =>
  ...
  const handleDelete = async () => {
    await fetch(apiUrl(`/api/audits/${auditId}/industrializations/${indId}`), {
      method: 'DELETE',
      credentials: 'include',
    });
  ...
  const callAi = async (kind: 'milestones' | 'cost') => {
    ...
    try {
      const path =
        kind === 'milestones' ? 'suggest-milestones' : 'bootstrap-cost';
      const res = await fetch(
        apiUrl(`/api/audits/${auditId}/industrializations/${indId}/ai/${path}`),
        {
          method: 'POST',
          credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
      ...
      if (kind === 'cost' && typeof data.rationale === 'string')
        setCostRationale(data.rationale);
    } catch (e: unknown) {
  ...
  const suggestMaintenance = async () => {
    ...
    try {
      const res = await fetch(
        apiUrl(
          `/api/audits/${auditId}/industrializations/${indId}/ai/suggest-maintenance`,
        ),
        {
          method: 'POST',
          credentials: 'include',
      ...
      if (typeof data.rationale === 'string')
        setMaintenanceRationale(data.rationale);
      const existingMaint =
        ind.cost.recurringAnnual?.maintenance ?? ({} as any);
      ...
      const existingDrivers = (existingMaint.drivers ?? {}) as Record<
        string,
        unknown
      >;
      const mergedDrivers: Record<string, unknown> = { ...existingDrivers };
      for (const k of Object.keys(incomingDrivers)) {
        if (existingDrivers[k] === undefined)
          mergedDrivers[k] = incomingDrivers[k];
      }
  ...
  const aggregateProgress =
    total === 0
      ? 0
      : Math.round(
          milestonesAll.reduce((s, m) => s + milestonePct(m as any), 0) / total,
        );
  const progress =
    ...
    <div className="max-w-5xl mx-auto space-y-5">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <span className="font-mono text-sm font-bold text-indu">
        ...
        <div className="flex items-center gap-3">
          {ind.isArchived && (
            <span className="text-[11px] uppercase tracking-wide font-semibold text-muted bg-smoke border border-border rounded px-2 py-0.5">
              Archived
            </span>
          )}
          ...
          {canEdit && (
            <button
              ...
              className="text-muted hover:text-blue-aria"
              title={ind.isArchived ? 'Unarchive' : 'Archive'}
            >
              {ind.isArchived ? (
      ...
      {(() => {
        ...
        if (proc && typeof proc === 'object' && proc.procId)
          nodes.push({
            kind: 'process',
            code: proc.procId,
            label: proc.name,
            href: `/audits/${auditId}/processes/${(proc as any)._id ?? ''}`,
          });
        if (uc && typeof uc === 'object' && uc.cuId)
          nodes.push({
            kind: 'usecase',
            code: uc.cuId,
            label: uc.description,
            href: `/audits/${auditId}/usecases`,
          });
        if (poc && typeof poc === 'object' && poc.pocId)
          nodes.push({
            kind: 'poc',
            code: poc.pocId,
            label: poc.name,
            href: `/audits/${auditId}/pocs/${(poc as any)._id ?? ''}`,
          });
        nodes.push({
          kind: 'industrialization',
          code: ind.industrializationId,
          label: ind.name,
        });
      ...
      <fieldset disabled={!canEdit} className="contents">
        <div className="card p-4 grid md:grid-cols-3 gap-4">
          <div className="md:col-span-2">
            ...
            <input
              value={ind.name ?? ''}
              onChange={(e) => {
                const next = { ...ind, name: e.target.value };
                setInd(next);
          ...
          <div>
            ...
            <select
              value={ind.status}
              onChange={(e) =>
                updateStatus(e.target.value as IndustrializationStatus)
              }
              className="form-input"
            >
              {(
            ...
            {statusError && (
              <p className="text-[11px] text-red-sov mt-1">{statusError}</p>
            )}
          </div>
          <div className="md:col-span-3">
            ...
            <input
              ...
              onChange={(e) => {
                const next = { ...ind, statusReason: e.target.value };
                setInd(next);
                trigger({ statusReason: e.target.value });
              }}
          ...
          <div className="card p-5 space-y-4">
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  value={ind.plan?.ownerBusiness ?? ''}
                  onChange={(e) => updatePlan('ownerBusiness', e.target.value)}
                  className="form-input"
                />
              </div>
              <div>
                ...
                <input
                  value={ind.plan?.ownerTechnical ?? ''}
                  onChange={(e) => updatePlan('ownerTechnical', e.target.value)}
                  className="form-input"
                />
              </div>
              <div>
                ...
                <input
                  type="date"
                  value={dateInput(ind.plan?.startDate)}
                  onChange={(e) =>
                    updatePlan('startDate', e.target.value || null)
                  }
                  className="form-input"
                />
              </div>
              <div>
                ...
                <input
                  type="date"
                  value={dateInput(ind.plan?.targetGoLiveDate)}
                  onChange={(e) =>
                    updatePlan('targetGoLiveDate', e.target.value || null)
                  }
                  className="form-input"
                />
              </div>
              <div>
                ...
                <input
                  type="date"
                  value={dateInput(ind.plan?.actualGoLiveDate)}
                  onChange={(e) =>
                    updatePlan('actualGoLiveDate', e.target.value || null)
                  }
                  className="form-input"
                />
              </div>
            ...
            <div>
              <label className="form-label">
                Scope (sites, teams, geographies)
              </label>
              <textarea
                value={ind.plan?.scope ?? ''}
                onChange={(e) => updatePlan('scope', e.target.value)}
            ...
            <div>
              ...
              <textarea
                value={ind.plan?.dependencies ?? ''}
                onChange={(e) => updatePlan('dependencies', e.target.value)}
                className="form-textarea"
                rows={2}
              />
            </div>
            <div>
              <label className="form-label">
                Sovereignty constraints (carried from B2 / POC)
              </label>
              <textarea
                value={ind.plan?.sovereigntyConstraints ?? ''}
                onChange={(e) =>
                  updatePlan('sovereigntyConstraints', e.target.value)
                }
                className="form-textarea"
                rows={2}
              />
        ...
        {activeTab === 'milestones' && (
          <div className="card p-5 space-y-3">
            ...
            {aiError && aiBusy === null && (
              <p className="text-[11px] text-red-sov bg-red-sov-light rounded p-2">
                {aiError}
              </p>
            )}
                        ...
                        <div className="md:col-span-6 flex gap-1 flex-wrap">
                          {MILESTONE_STATUSES.map((s) => (
                            <button
                              key={s.key}
                              onClick={() =>
                                updateMilestone(m.id, 'status', s.key)
                              }
        ...
        {activeTab === 'cost' && (
          <CostEditor
            ...
            onChange={(patch) => {
              const next = {
                ...ind,
                cost: { ...ind.cost, ...patch } as IndustrializationCost,
              };
              setInd(next);
              trigger({ cost: next.cost });
            }}
            ...
            inheritedFromPoc={(ind.aiGeneratedFields ?? []).includes(
              'cost.recurringAnnual.computeBreakdown',
            )}
          />
        )}
        {activeTab === 'roi' && (
          <ROIEditor
            ...
            onChange={(patch) => {
              const next = {
                ...ind,
                roi: { ...ind.roi, ...patch } as IndustrializationROI,
              };
              setInd(next);
        ...
        {activeTab === 'production' && (
          <div className="card p-5 space-y-3">
            <div>
              ...
              <textarea
                value={ind.production?.monitoredKpis ?? ''}
                onChange={(e) => {
                  const v = e.target.value;
                  const next = {
                    ...ind,
                    production: { ...ind.production, monitoredKpis: v },
                  };
                  setInd(next);
                  trigger({ production: next.production });
                }}
                className="form-textarea"
                rows={3}
              />
            ...
            <div>
              ...
              <textarea
                value={ind.production?.incidentsLog ?? ''}
                onChange={(e) => {
                  const v = e.target.value;
                  const next = {
                    ...ind,
                    production: { ...ind.production, incidentsLog: v },
                  };
                  setInd(next);
                  trigger({ production: next.production });
                }}
            ...
            <div>
              <label className="form-label">
                Decommissioning plan (legacy system)
              </label>
              <textarea
                value={ind.production?.decommissioningPlan ?? ''}
                onChange={(e) => {
                  const v = e.target.value;
                  const next = {
                    ...ind,
                    production: { ...ind.production, decommissioningPlan: v },
                  };
                  setInd(next);
                  trigger({ production: next.production });
                }}
