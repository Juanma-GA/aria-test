    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted mt-0.5">
          Download audit data as CSV files (open directly in Excel)
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {exports.map((exp) => (
          <div
            key={exp.title}
            className="bg-white border border-border rounded-sm p-6 flex flex-col gap-4"
          >
      ...
      <div className="bg-white border border-border rounded-sm p-6">
        <h2 className="font-display font-semibold text-text mb-1">
          Compound ID Migration
        </h2>
        <p className="text-sm text-muted mb-4">
          Reassigns all existing records to use hierarchical compound codes:
          <span className="ml-1 font-mono text-xs text-blue-aria">
            AUD-001 → AUD-001-P01 → AUD-001-P01-C01 → POC-AUD-001-P01-C01-01
          </span>
        </p>
        <button
