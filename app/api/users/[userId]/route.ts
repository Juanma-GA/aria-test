...
export async function PATCH(
  ...
  try {
    ...
    if (email) {
      const existing = await User.findOne({ email: email.toLowerCase(), _id: { $ne: params.userId } });
      if (existing) {
        return NextResponse.json({ error: 'Email already in use' }, { status: 409 });
      }
    }

    const user = await User.findByIdAndUpdate(params.userId, update, { new: true }).select('-passwordHash').lean();
