...
interface AuditUseCase {
  ...
  processId: { _id: string; procId: string; name?: string; b1?: { profiles: { id: string; count: number }[] } } | null;
  score?: {
    dimensions: {
...
function calcRoi(uc: AuditUseCase): string | null {
  const saved = (uc.timeSavedPerProfile ?? []).reduce((s, e) => s + e.hoursPerExecution, 0);
    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted mt-0.5">All AI opportunities in this audit</p>
        </div>
        <a
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-slate-50">
                {['ID', 'Description', 'Process', 'AI Types', 'People', 'Score', 'Category', 'Status', 'ROI'].map(h => (
            ...
            <tbody>
              {filtered.map(uc => {
                ...
                const editHref = proc ? `/audits/${auditId}/processes/${proc._id}/b5?edit=${uc._id}` : null;
                return (
                  <tr
                    key={uc._id}
