...
export async function requireAuditAccess(
  ...
  if (caller.role === 'viewer') {
    if (level === 'view')
      return { user: caller, audit, effectiveRole: 'viewer' };
    return NextResponse.json(
      { error: 'Forbidden — viewer is read-only' },
      { status: 403 },
    );
  }
  ...
  if (!teamRole) {
    return NextResponse.json(
      { error: 'Forbidden — not a team member of this audit' },
      { status: 403 },
    );
  }
  ...
  if (level === 'edit') {
    if (teamRole === 'owner' || teamRole === 'editor')
      return { user: caller, audit, effectiveRole: teamRole };
    return NextResponse.json(
