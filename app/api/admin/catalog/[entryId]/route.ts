...
export async function GET(
  ...
  try {
    await dbConnect();
    const item = await Catalog.findById(entryId).lean();
...
export async function PATCH(
  ...
  try {
    await dbConnect();
    const existing = await Catalog.findById(entryId);
    ...
    if (typeof update.name === 'string' && update.name) {
      const clash = await Catalog.findOne({
        kind: existing.kind, name: update.name, _id: { $ne: entryId },
      });
      if (clash) return NextResponse.json({ error: 'Another entry with that name exists' }, { status: 409 });
    }

    const updated = await Catalog.findByIdAndUpdate(entryId, update, { new: true }).lean();
...
export async function DELETE(
  ...
  try {
    await dbConnect();
    const item = await Catalog.findById(entryId);
    if (!item) return NextResponse.json({ error: 'Not found' }, { status: 404 });
