...
const PHASE_COLORS = { design: 'slate', execution: 'blue', evaluation: 'amber', closed: 'green' } as const;
const DECISION_COLORS = { go: 'green', go_conditional: 'teal', no_go_redesign: 'amber', no_go_discard: 'red', paused: 'purple', pending: 'slate' } as const;
const DECISION_LABELS = { go: 'GO', go_conditional: 'GO Conditional', no_go_redesign: 'No-Go – Redesign', no_go_discard: 'No-Go – Discard', paused: 'Paused', pending: 'Pending' };
    ...
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => router.push(`/audits/${auditId}/pocs/new`)} className="btn-primary flex items-center gap-1">
        ...
        <div className="card overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-smoke border-b border-border">
              <tr>
                {['POC-ID', 'Name', 'Use Case', 'Process', 'Responsible', 'Start', 'Deadline', 'Phase', 'Decision', ''].map(h => (
            ...
            <tbody>
              {pocs.map(poc => {
                ...
                const procLabel = pd?.procId && pd?.name
                  ? `${pd.procId} · ${pd.name}`
                  : pd?.procId ?? '—';
                return (
                  <tr key={poc._id} className="border-b border-border/50 hover:bg-smoke/30 cursor-pointer" onClick={() => router.push(`/audits/${auditId}/pocs/${poc._id}`)}>
                    ...
                    <td className="py-3 px-3">
                      <Badge variant={DECISION_COLORS[poc.decision?.decision || 'pending']}>
