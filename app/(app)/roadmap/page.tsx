...
export default function GlobalRoadmapPage() {
  ...
  const [phaseFilter, setPhaseFilter] = useState<'all' | 'design' | 'execution' | 'evaluation' | 'closed'>('all');
  ...
  const phaseFiltered = phaseFilter === 'all' ? datedPocs : datedPocs.filter(p => p.phase === phaseFilter);
  ...
  const auditGroups = new Map<string, { name: string; client?: string; pocs: EnrichedPOC[] }>();
  for (const poc of phaseFiltered) {
    const aid = poc.audit?._id ?? poc.auditId ?? 'unknown';
    if (!auditGroups.has(aid)) {
      auditGroups.set(aid, { name: poc.audit?.name ?? 'Unknown Audit', client: poc.audit?.client, pocs: [] });
    }
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-muted mt-0.5">POC timeline across all audits — {pocs.length} POCs</p>
      </div>
          ...
          <div className="flex items-center gap-5 px-4 pt-3 pb-2 border-b border-border text-xs text-muted flex-wrap">
            ...
            {Object.entries(PHASE_VARIANTS).map(([phase]) => (
              <div key={phase} className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded-sm inline-block" style={{ backgroundColor: PHASE_COLORS[phase] }} />
                <span className="capitalize">{phase}</span>
              </div>
            ))}
            <span className="ml-4 font-medium text-text">Milestones:</span>
            {[['done', '#22c55e', '✓ Done'], ['pending', '#94a3b8', '◆ Pending'], ['missed', '#ef4444', '◆ Missed']].map(([k, c, l]) => (
              <div key={k} className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 inline-block rotate-45" style={{ backgroundColor: c as string }} />
                <span>{l}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ width: LABEL_W + ganttW }}>
            ...
            {[...auditGroups.entries()].map(([auditId, group]) => (
              <div key={auditId}>
                ...
                <div className="flex bg-slate-50/80 border-b border-border/40">
                  <div className="flex-shrink-0 px-4 py-1.5 border-r border-border" style={{ width: LABEL_W }}>
                  ...
                  <div className="flex" style={{ width: ganttW }}>
                    {months.map((_, i) => (
                      <div key={i} style={{ width: monthWidth, flexShrink: 0 }} className="border-r border-border/10" />
                    ))}
                  </div>
                </div>
                    ...
                    <div key={poc._id}
                      ...
                      <div className="flex-shrink-0 px-4 py-3 border-r border-border flex flex-col justify-center gap-0.5" style={{ width: LABEL_W }}>
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          <Badge variant={PHASE_VARIANTS[poc.phase]}>{poc.phase}</Badge>
                        </div>
                        {(poc.name || objective) && (
                          <p className="text-xs text-text font-semibold truncate" title={poc.name || objective}>{poc.name || objective}</p>
                        )}
                        {poc.name && objective && poc.name !== objective && (
                          <p className="text-[10px] text-muted truncate" title={objective}>{objective}</p>
                        )}
                      ...
                      <div className="relative py-3" style={{ width: ganttW }}>
                        ...
                        {showToday && (
                          <div className="absolute top-0 bottom-0 pointer-events-none z-10"
                            style={{ left: `${todayPct}%`, width: 1, backgroundColor: '#ef4444', opacity: 0.5 }} />
                        )}
                        ...
                        <div
                          ...
                          style={{
                            ...
                            minWidth: 6,
                            height: 20,
                            top: milestones.length > 0 ? 'calc(50% - 22px)' : 'calc(50% - 10px)',
                          ...
                          return (
                            ...
                            >
                              ...
                              <span
                                ...
                                style={{ color, maxWidth: 60, overflow: 'hidden', textOverflow: 'ellipsis', display: 'block' }}
                                title={ms.name}
                              >
            ...
            {showToday && (
              <div className="relative h-4 border-t border-border/30">
                <div className="absolute -translate-x-1/2 text-[9px] text-red-500 font-medium whitespace-nowrap"
            ...
            {/* POCs without dates */}
            {pocs.filter(p => !p.design?.startDate || !p.design?.deadlineDate).length > 0 && (
              <div className="px-4 py-3 text-xs text-muted border-t border-border bg-slate-50">
                {pocs.filter(p => !p.design?.startDate || !p.design?.deadlineDate).length} POC(s) not shown — no start/deadline dates set.
              </div>
            )}
          </div>
        </div>
