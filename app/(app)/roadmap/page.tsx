...
export default function GlobalRoadmapPage() {
  ...
  const [collapsedAudits, setCollapsedAudits] = useState<Set<string>>(
    new Set(),
  );
  ...
  useEffect(() => {
    Promise.all([
      fetch(apiUrl('/api/pocs'), { credentials: 'include' }).then((r) =>
        r.json(),
      ),
      fetch(apiUrl('/api/industrializations'), { credentials: 'include' }).then(
        (r) => r.json(),
      ),
    ])
      .then(([p, i]) => {
  ...
  for (const v of availableAudits.values())
    if (v.client) availableClients.add(v.client);

  const auditMatches = (auditId?: string, clientName?: string) => {
    if (auditFilter.size > 0 && (!auditId || !auditFilter.has(auditId)))
      return false;
    if (clientFilter.size > 0 && (!clientName || !clientFilter.has(clientName)))
      return false;
    return true;
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <MultiSelectFilter
          label="Audits"
          options={[...availableAudits.entries()].map(([id, v]) => ({
            id,
            label: v.name,
            sublabel: v.client,
          }))}
        ...
        {groupBy === 'audit' && (
          <div className="flex gap-1 ml-auto text-[11px]">
            <button
              onClick={collapseAll}
              className="text-muted hover:text-text px-2 py-1 border border-border rounded-sm"
            >
              Collapse all
            </button>
            <button
...
function renderByAudit({
  ...
  for (const poc of visiblePocs) {
    ...
    if (!auditGroups.has(aid))
      auditGroups.set(aid, {
        ...
        startDate: poc.audit?.startDate,
        targetEndDate: poc.audit?.targetEndDate,
        pocs: [],
        orphanInds: [],
      });
  ...
  for (const ind of visibleInds) {
    const pocId =
      typeof ind.pocId === 'object' ? ind.pocId._id : String(ind.pocId);
    if (visiblePocIds.has(pocId)) continue; // will render under its POC
    const aid = ind.audit?._id ?? ind.auditId ?? 'unknown';
    if (!auditGroups.has(aid))
      auditGroups.set(aid, {
        name: ind.audit?.name ?? 'Unknown Audit',
        client: ind.audit?.client,
        startDate: ind.audit?.startDate,
        targetEndDate: ind.audit?.targetEndDate,
        pocs: [],
        orphanInds: [],
      });
  ...
  return [...auditGroups.entries()].map(([auditId, group]) => {
    ...
    const totalRows =
      group.pocs.length +
      group.orphanInds.length +
      group.pocs.filter((p) => indByPoc.has(p._id)).length;
    ...
    const hasAuditRange =
      auditStart && auditEnd && auditEnd.getTime() > auditStart.getTime();
    const auditLeftPct = hasAuditRange
      ? dateToPct(auditStart!, ganttStart, totalMonths)
      : 0;
    const auditRightPct = hasAuditRange
      ? dateToPct(auditEnd!, ganttStart, totalMonths)
      : 0;
    ...
    return (
      <div key={auditId}>
        ...
        >
          <div
            className="flex-shrink-0 px-4 py-1.5 border-r border-border flex items-center gap-1.5"
            style={{ width: LABEL_W }}
          >
            {isCollapsed ? (
          ...
          <div
            ...
            style={{ width: ganttW, minHeight: 28 }}
          >
            {months.map((_, i) => (
              <div
                key={i}
            ...
            {hasAuditRange && (
              ...
              >
                <span className="text-[9px] font-medium text-blue-aria truncate">
                  Audit duration
                </span>
              </div>
            )}
          ...
          group.pocs.map((poc) => {
            ...
            return (
              <div key={poc._id}>
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      <Badge variant={POC_PHASE_VARIANTS[poc.phase]}>
                        {poc.phase}
                      </Badge>
                    </div>
                    {(poc.name || objective) && (
                    ...
                    <div className="text-[10px] text-muted truncate">
                      {poc.useCase?.cuId && (
                        <span className="font-mono">{poc.useCase.cuId}</span>
                      )}
                ...
                {ind &&
                  (() => {
                    const indMs = (ind.milestones ?? []).filter(
                      (m) => m.dueDate,
                    );
                    return (
                      <div
                        ...
                        style={{
                          minHeight: indMs.length > 0 ? 60 : 44,
                        ...
                        onClick={(e) => {
                          ...
                          ind.audit &&
                            router.push(
                              `/audits/${ind.audit._id}/industrializations/${ind._id}`,
                            );
                        }}
                      >
                        <div
                          className="flex-shrink-0 pl-8 pr-4 py-2 border-r border-border flex flex-col justify-center gap-0.5"
                          style={{ width: LABEL_W }}
                        >
                          <div className="flex items-center gap-1.5 flex-wrap">
          ...
          group.orphanInds.map((ind) => {
            ...
            return (
              ...
              >
                ...
                >
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <span className="font-mono text-xs font-bold text-indu">
                      {ind.industrializationId}
                    </span>
                    <Badge variant={IND_STATUS_VARIANTS[ind.status]}>
                      {INDUSTRIALIZATION_STATUS_LABELS[ind.status]}
                  ...
                  {ind.name && (
                    <p className="text-xs text-text truncate" title={ind.name}>
                      {ind.name}
                    </p>
                  )}
                  {ind.poc?.pocId && (
...
function renderByUseCase({
  ...
  for (const poc of visiblePocs) {
    ...
    if (!ucGroups.has(ucId))
      ucGroups.set(ucId, {
        cuId: poc.useCase?.cuId ?? '—',
        description: poc.useCase?.description,
        auditName: poc.audit?.name,
        pocs: [],
        inds: [],
      });
  ...
  for (const ind of visibleInds) {
    const ucId =
      typeof ind.useCaseId === 'object'
        ? ind.useCaseId._id
        : String(ind.useCaseId);
    if (!ucGroups.has(ucId))
      ucGroups.set(ucId, {
        cuId: ind.useCase?.cuId ?? '—',
        description: ind.useCase?.description,
        auditName: ind.audit?.name,
        pocs: [],
        inds: [],
      });
    ...
    const onClick = () => {
      const target =
        ind && ind.audit
          ? `/audits/${ind.audit._id}/industrializations/${ind._id}`
          : poc && poc.audit
            ? `/audits/${poc.audit._id}/pocs/${poc._id}`
            : null;
    ...
    return (
      ...
      >
        ...
        >
          ...
          {group.auditName && (
            <p className="text-[10px] text-muted truncate">{group.auditName}</p>
          )}
        </div>
        <div className="relative py-3" style={{ width: ganttW }}>
          <GridAndToday
          ...
          {options.length === 0 ? (
            ...
            <>
              {selected.size > 0 && (
                <button
                  onClick={clear}
                  className="w-full text-left px-3 py-1.5 text-[11px] text-blue-aria hover:bg-smoke border-b border-border"
                >
                  Clear selection
                </button>
              )}
              {options.map((opt) => (
                <label
                  key={opt.id}
                  className="flex items-start gap-2 px-3 py-1.5 hover:bg-smoke cursor-pointer text-xs"
                >
                  <input
                    type="checkbox"
