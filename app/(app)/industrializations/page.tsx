...
export default function GlobalIndustrializationsPage() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(
      apiUrl(`/api/industrializations${showArchived ? '?archived=true' : ''}`),
      { credentials: 'include' },
    )
      .then((r) => r.json())
      .then((data) => {
    ...
    <div className="space-y-5">
        ...
        <div className="flex flex-wrap gap-1 bg-white border border-border rounded-sm p-1">
          {STATUS_FILTERS.map((f) => (
            ...
            >
              {f === 'all'
                ? `All (${counts[f]})`
                : `${INDUSTRIALIZATION_STATUS_LABELS[f as IndustrializationStatus]} (${counts[f]})`}
            </button>
          ))}
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="card p-12 text-center text-muted text-sm">
          No industrializations match the current filters.
        </div>
      ) : (
        <div className="card overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((i) => {
                const target = i.plan?.targetGoLiveDate
                  ? new Date(i.plan.targetGoLiveDate).toLocaleDateString()
                  : '—';
                return (
                  <tr
                    key={i._id}
                    ...
                    onClick={() =>
                      i.audit &&
                      router.push(
                        `/audits/${i.audit._id}/industrializations/${i._id}`,
                      )
                    }
                  >
                    ...
                    <td className="py-3 px-3 text-xs text-text font-medium">
                      {i.name || <span className="text-muted">—</span>}
                    </td>
                    ...
                    <td className="py-3 px-3">
                      <Badge variant={STATUS_VARIANTS[i.status]}>
                        {INDUSTRIALIZATION_STATUS_LABELS[i.status]}
                      </Badge>
                    </td>
