...
export async function PATCH(
  ...
  try {
    ...
    if (!role || !VALID_ROLES.includes(role)) {
      return NextResponse.json({ error: 'role (owner|editor|viewer) is required' }, { status: 400 });
    }
    ...
    if (role !== 'owner' && team[idx].role === 'owner') {
      ...
      if (ownerCount <= 1) {
        return NextResponse.json({ error: 'Cannot demote the last owner' }, { status: 422 });
      }
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (target.role === 'owner') {
      const ownerCount = team.filter((m: any) => m.role === 'owner').length;
      if (ownerCount <= 1) {
        return NextResponse.json({ error: 'Cannot remove the last owner' }, { status: 422 });
      }
    }
