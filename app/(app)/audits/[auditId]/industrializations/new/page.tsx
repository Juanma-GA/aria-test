...
export default function NewIndustrializationPage() {
  ...
  const handleCreate = async () => {
    if (!selectedPoc) {
      setError('Select a validated POC.');
      return;
    }
    setSaving(true);
    setError('');
    try {
      const res = await fetch(
        apiUrl(
          `/api/audits/${auditId}/industrializations/from-poc/${selectedPoc}`,
        ),
        {
          method: 'POST',
          credentials: 'include',
      ...
      if (!res.ok)
        throw new Error(data.error || 'Failed to create industrialization');
      ...
      <div className="card p-6 space-y-4">
        <div>
          ...
          <p className="text-[11px] text-muted mb-1.5">
            Only POCs with decision GO or GO Conditional and not yet
            industrialized are listed.
          </p>
          {available.length === 0 ? (
            <p className="text-sm text-muted">
              No eligible POCs. Validate a POC (GO / GO Conditional) before
              creating an industrialization.
            </p>
          ) : (
            <select
