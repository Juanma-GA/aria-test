...
export default function POCDetailPage() {
  ...
  const updateComputeCost = (field: string, value: unknown) => {
    ...
    const next = {
      ...poc,
      computeCost: { ...(poc as any).computeCost, [field]: value },
    };
    setPoc(next as POC);
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="space-y-4">
        ...
        {(() => {
          ...
          const cloudCost =
            annualReps > 0
              ? (((cc.inputTokensPerExec ?? 1000) * annualReps) / 1_000_000) *
                  (cc.pricePerMInputTokens ?? 2) +
                (((cc.outputTokensPerExec ?? 500) * annualReps) / 1_000_000) *
          ...
          const hybridCost =
            onPremCost * onPremPct + cloudCost * (1 - onPremPct);

          // Break-even
          const monthlyCloudSavingVsOnPrem =
            (cloudCost - (electricity + maintenance)) / 12;
          const breakevenMonths =
            monthlyCloudSavingVsOnPrem > 0
              ? Math.round(totalGpuCost / monthlyCloudSavingVsOnPrem)
              : null;
          ...
          const updateSub = (
            ...
            setSubs(
              subs.map((s, idx) => (idx === i ? { ...s, [field]: value } : s)),
            );
                    ...
                    onClick={async (e) => {
                      ...
                      try {
                        ...
                        if (data.estimates) {
                          ...
                          const updated = { ...cc };
                          if (e2.pricePerMInputTokens != null)
                            updated.pricePerMInputTokens =
                              e2.pricePerMInputTokens;
                          if (e2.pricePerMOutputTokens != null)
                            updated.pricePerMOutputTokens =
                              e2.pricePerMOutputTokens;
                          if (e2.inputTokensPerExec != null)
                            updated.inputTokensPerExec = e2.inputTokensPerExec;
                          if (e2.outputTokensPerExec != null)
                            updated.outputTokensPerExec =
                              e2.outputTokensPerExec;
                          if (e2.avgResponseTimeSec != null)
                            updated.avgResponseTimeSec = e2.avgResponseTimeSec;
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="bg-slate-50 rounded p-2.5 space-y-1.5 text-xs text-muted">
                        ...
                        <div className="flex gap-4 text-[11px] flex-wrap">
                          <span>
                            Amortization: €
                            {Math.round(amortization).toLocaleString()}/yr
                          </span>
                          <span>
                            Electricity: €
                            {Math.round(electricity).toLocaleString()}/yr
                          </span>
                          <span>
                            Maintenance: €
                            {Math.round(maintenance).toLocaleString()}/yr
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    {subs.length > 0 && (
                      <div className="space-y-2">
                        ...
                        {subs.map((sub, i) => (
                          <div
                            key={i}
                            className="grid grid-cols-[1fr_72px_80px_72px_24px] gap-2 items-center"
                          >
                            <input
                            ...
                            <span className="text-sm font-medium text-text text-right">
                              €
                              {Math.round(
                                sub.users * sub.monthlyPerUser * 12,
                              ).toLocaleString()}
                            </span>
                            <button
                              onClick={() => removeSub(i)}
                        ...
                        <div className="flex justify-between text-sm font-semibold text-text pt-2 border-t border-border">
                          ...
                          <span>
                            €{Math.round(subscriptionsCost).toLocaleString()}
                          </span>
                        </div>
                      </div>
                    )}
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        {breakevenMonths !== null && (
                          <div className="rounded p-3 border border-amber-sov bg-amber-sov/5 col-span-2">
                            <div className="text-[10px] text-muted uppercase tracking-wide mb-1">
                              Break-Even (Cloud vs On-Prem)
                            </div>
                            <div className="text-lg font-bold text-amber-sov">
                              {breakevenMonths} months
                            </div>
                            <div className="text-[10px] text-muted">
                              until on-prem investment recovers
                            </div>
                          </div>
                        )}
                      </div>
                      ...
                      <tr className="border-b border-border">
                        {["Milestone", "Due Date", "Status", "Notes", ""].map(
                          (h) => (
                            <th
                              key={h}
                              className="text-left py-2 px-2 text-xs font-medium text-muted"
                            >
                              {h}
                            </th>
                          ),
                ...
                <div className="mt-3">
                  ...
                  <div className="w-full h-2 bg-slate-200 rounded-full">
                    <div
                      ...
                      style={{
                        width: `${Math.round((doneMilestones / milestones.length) * 100)}%`,
                      }}
                    />
                  </div>
                </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => {
                    const n = {
                      ...poc,
                      decision: { ...poc.decision, nextSteps: e.target.value },
                    };
                    setPoc(n as POC);
                    trigger({ decision: n.decision });
                  }}
